...
from new_templates import xSecReader, getSumW, scaleSumW, collate
...
def make_wtemplates(collated, temptype, lumi=1, systs=True, region='wtag'):
    ...
    for proc in tqdm(proc_names, desc='Samples:'):
        ...
        for syst in tqdm(sys_list, desc='Syst', leave=False):
            if region not in h_obj.axes['region']:
                continue
            base_cuts = {'region': region, 'systematic': syst, 'pt': s[200j:len:sum]}
            if temptype == 'cvl':
...
if __name__ == "__main__":
    ...
    parser.add_argument("--systs", type=str2bool, default='True', choices={True, False}, help='Process systematics')
    ...
    parser.add_argument("--types", default=None, help="Pass multiple comma separated types from --type to process at once.")
    ...
    for template_type in template_types:
        ...
        templates = make_wtemplates(collated, template_type, lumi=lumi_mu[args.year], systs=args.systs, region=args.region)

        # Stack them to matched/unmatched
        merged_dict = {}
        samples = ['tqq', 'stqq', 'wln', 'qcd']
        ...
        if args.output is None:
            ...
            template_file = f"{args.identifier}/wtemplates_{template_type}.root"
        else:
        ...
        if args.plot:
            ...
            for clip in [True, False]:
                for pf in ["Pass", "Fail"]:
                    ...
                    fig.savefig(f"{args.identifier}/plots/wtemplates_{template_type}_templates_{pf.lower()}{_cl}.png")
                    fig.savefig(f"{args.identifier}/plots/wtemplates_{template_type}_templates_{pf.lower()}{_cl}.pdf")
                    ...
                    hep.cms.label('Preliminary', year=args.year, data=True)
                    _cl = "_clip" if clip else ""
                    fig.savefig(f"{args.identifier}/plots/wtemplates_{template_type}_samples_{pf.lower()}{_cl}.png")
                    fig.savefig(f"{args.identifier}/plots/wtemplates_{template_type}_samples_{pf.lower()}{_cl}.pdf")

                    # Make RAW sample plots
                    if True:
                        soutput['data_obs'] = soutput['SingleMuon']
                        raw_templates = make_wtemplates(soutput, template_type, lumi=lumi_mu[args.year], systs=args.systs, region=args.region)
                        fig, ax = plt.subplots()
                        allsamples = [sname for sname in list(soutput.keys())[::-1] if sname not in ['SingleMuon', 'JetHT']]
                        mc_hists = [raw_templates[f'{sname}_{pf.lower()}_nominal'] for sname in allsamples if f'{sname}_{pf.lower()}_nominal' in raw_templates]
                        mc = [mh[{'genflavor': s[::sum]}] for mh in mc_hists]
                        ...
                        hep.cms.label('Preliminary', year=args.year, data=True)
                        _cl = "_clip" if clip else ""
                        fig.savefig(f"{args.identifier}/plots/wtemplates_{template_type}_raw_{pf.lower()}{_cl}.png")
                        fig.savefig(f"{args.identifier}/plots/wtemplates_{template_type}_raw_{pf.lower()}{_cl}.pdf")
